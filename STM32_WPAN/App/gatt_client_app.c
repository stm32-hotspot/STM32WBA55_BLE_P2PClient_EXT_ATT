...
static void P2Pclient_write_char_ext(void);
static void P2Pclient_read_char_ext(void);
static void gatt_parse_notification_ext(aci_gatt_notification_ext_event_rp0 *p_evt);
/* USER CODE END PFP */
...
void GATT_CLIENT_APP_Init(void)
{
  ...
  UTIL_SEQ_RegTask(1U << CFG_TASK_P2PC_WRITE_CHAR_EXT_ID, UTIL_SEQ_RFU, P2Pclient_write_char_ext);
  UTIL_SEQ_RegTask(1U << CFG_TASK_P2PC_READ_CHAR_EXT_ID, UTIL_SEQ_RFU, P2Pclient_read_char_ext);
  ...
  switch (event_pckt->evt)
  {
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
    {
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        {
          aci_gatt_notification_ext_event_rp0 *p_evt_rsp = (void*)p_blecore_evt->data;
          gatt_parse_notification_ext((aci_gatt_notification_ext_event_rp0 *)p_evt_rsp);
        }
        break; /* ACI_GATT_NOTIFICATION_EXT_VSEVT_CODE */
        ...
        case ACI_ATT_EXCHANGE_MTU_RESP_VSEVT_CODE:
        {
          ...
          tBleStatus ret = hci_le_set_data_length(exchange_mtu_resp->Connection_Handle,251,2120);
          if (ret != BLE_STATUS_SUCCESS)
          {
            LOG_INFO_APP("  Fail   : hci_le_set_data_length command, result: 0x%x \n\r", ret);
          }
          else
          {
            LOG_INFO_APP("  Success: hci_le_set_data_length command\n\r");
          }
        ...
        {
          LOG_INFO_APP("-- GATT : ACI_ATT_PREPARE_WRITE_RESP_VSEVT_CODE \n");
          UTIL_SEQ_SetEvt(1<< CFG_PREPARE_WRITE_EVT_RSP_ID);
        }
        break; /* ACI_ATT_PREPARE_WRITE_RESP_VSEVT_CODE */
        case ACI_ATT_EXEC_WRITE_RESP_VSEVT_CODE:
        {
          LOG_INFO_APP("-- GATT : ACI_ATT_EXEC_WRITE_RESP_VSEVT_CODE \n");
          UTIL_SEQ_SetEvt(1<< CFG_EXECUTE_WRITE_EVT_RSP_ID);
        }
        break; /* ACI_ATT_EXEC_WRITE_RESP_VSEVT_CODE */
        case ACI_ATT_READ_RESP_VSEVT_CODE:
        {
          ...
          LOG_INFO_APP("-- P2P APPLICATION CLIENT : READ RESPONSE RECEIVED\r\n");
          LOG_INFO_APP("** Read Data (Length: %d): 0x", read_resp->Event_Data_Length);
          for (uint8_t i = 0; i < read_resp->Event_Data_Length; i++)
          {
            LOG_INFO_APP("%02X ", read_resp->Attribute_Value[i]);
          }
          LOG_INFO_APP("\n\n");
        }
        ...
        case ACI_GATT_READ_EXT_VSEVT_CODE:
        {
          ...
          read_ext = (aci_gatt_read_ext_event_rp0 *)p_blecore_evt->data;
          LOG_INFO_APP("-- P2P APPLICATION CLIENT : EXTENDED READ RESPONSE RECEIVED\r\n");
          LOG_INFO_APP("** Extended Read Data (Length: %d | Offset: %x): 0x", read_ext->Event_Data_Length, read_ext->Offset);
          for (uint8_t i = 0; i < read_ext->Event_Data_Length; i++)
          {
            LOG_INFO_APP("%02X ", read_ext->Attribute_Value[i]);
          }
          LOG_INFO_APP("\n\n");
        }
        break; /* ACI_GATT_READ_EXT_VSEVT_CODE */
        default:
          break;
  ...
  switch (p_Notif->Client_Evt_Opcode)
  {
    ...
    case P2P_NOTIFICATION_INFO_RECEIVED_EVT:
    {
      ...
      if (P2PLedLevel == 0x00)
      {
        ...
        LOG_INFO_APP("-- P2P APPLICATION CLIENT : NOTIFICATION RECEIVED\n");
        LOG_INFO_APP("** Notification Data: 0x%02X %02X\n\n", p_Notif->DataTransfered.p_Payload[0], p_Notif->DataTransfered.p_Payload[1]);
      }
      else if (P2PLedLevel == 0x01)
      {
        BSP_LED_On(LED_BLUE);
        LOG_INFO_APP("-- P2P APPLICATION CLIENT : NOTIFICATION RECEIVED\n");
        LOG_INFO_APP("** Notification Data: 0x%02X %02X\n\n", p_Notif->DataTransfered.p_Payload[0], p_Notif->DataTransfered.p_Payload[1]);
      ...
      {
        LOG_INFO_APP("-- P2P APPLICATION CLIENT : EXTENDED NOTIFICATION RECEIVED\n");
        LOG_INFO_APP("** Extended Notification Data (Length: %d): 0x", p_Notif->DataTransfered.length);
        for (uint16_t i = 0; i < p_Notif->DataTransfered.length; i++)
        {
...
static void gatt_parse_notification_ext(aci_gatt_notification_ext_event_rp0 *p_evt)
{
  LOG_INFO_APP("ACI_GATT_NOTIFICATION_EXT_VSEVT_CODE - ConnHdl=0x%04X, Attribute_Handle=0x%04X\n",
                p_evt->Connection_Handle,
                p_evt->Attribute_Handle);
  ...
  {
    LOG_INFO_APP("ACI_GATT_NOTIFICATION_EXT_VSEVT_CODE, failed handle not found in connection table !\n");
  }
...
static void P2Pclient_write_char_ext(void)
{
  ...
  for(index = 0; index < BLE_CFG_CLT_MAX_NBR_CB; index++)
  {
    if (a_ClientContext[index].state != GATT_CLIENT_APP_IDLE)
    {
      ret = aci_att_prepare_write_req(a_ClientContext[index].connHdl,a_ClientContext[index].P2PWriteToServerValueHdl,0,242,(uint8_t *)  payload);
      LOG_INFO_APP("aci_att_prepare_write_req : %x\n", ret);
      UTIL_SEQ_WaitEvt(1<< CFG_PREPARE_WRITE_EVT_RSP_ID);
      
      ret = aci_att_prepare_write_req(a_ClientContext[index].connHdl,a_ClientContext[index].P2PWriteToServerValueHdl,242,242,(uint8_t *)  payload+242);
      LOG_INFO_APP("aci_att_prepare_write_req : %x\n", ret);
      UTIL_SEQ_WaitEvt(1<< CFG_PREPARE_WRITE_EVT_RSP_ID);
      
      ret = aci_att_prepare_write_req(a_ClientContext[index].connHdl,a_ClientContext[index].P2PWriteToServerValueHdl,484,25,(uint8_t *)  payload+484);
      LOG_INFO_APP("aci_att_prepare_write_req : %x\n", ret);
      UTIL_SEQ_WaitEvt(1<< CFG_PREPARE_WRITE_EVT_RSP_ID);
      
      ret = aci_att_execute_write_req(a_ClientContext[index].connHdl,0x01);
      LOG_INFO_APP("aci_att_execute_write_req : %x\n", ret);
...
static void P2Pclient_read_char_ext(void)
{
  ...
  for(index = 0; index < BLE_CFG_CLT_MAX_NBR_CB; index++)
  {
    if (a_ClientContext[index].state != GATT_CLIENT_APP_IDLE)
    {
      ret = aci_gatt_read_char_value(a_ClientContext[index].connHdl, a_ClientContext[index].P2PWriteToServerValueHdl);
