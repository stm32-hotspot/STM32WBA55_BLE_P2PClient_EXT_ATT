...
void APP_BLE_Init(void)
{
  ...
  tBleStatus ret = aci_gatt_set_event_mask(0x0051DA6C);
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : aci_gatt_set_event_mask command, result: 0x%x\n", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: aci_gatt_set_event_mask command\n");
  }
      ...
      switch (ad_type)
      {
        ...
        case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
        {
          ...
          if(manufBlueST != UINT8_MAX)
          {
            ...
            LOG_INFO_APP("  P2P server, db addr 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
...
void APPE_Button2Action(void)
{
  ...
  {
    UTIL_SEQ_SetTask( 1u << CFG_TASK_P2PC_WRITE_CHAR_EXT_ID, CFG_SEQ_PRIO_0);
  }
...
{
  UTIL_SEQ_SetTask( 1u << CFG_TASK_P2PC_READ_CHAR_EXT_ID, CFG_SEQ_PRIO_0);
